...
func (s *TestSuite) TestExecute(c *C) {
	...
	for testName, testCase := range testCases {
		...
		if testCase.expectedErrorPrefix != "" {
			c.Assert(err, NotNil)
			c.Assert(strings.HasPrefix(err.Error(), testCase.expectedErrorPrefix), Equals, true, Commentf(TestErrErrorFmt, testName))
			continue
		}
		c.Assert(err, IsNil, Commentf(TestErrErrorFmt, testName, err))
		c.Assert(output, Equals, testCase.expected, Commentf(TestErrResultFmt, testName))
	}
}
