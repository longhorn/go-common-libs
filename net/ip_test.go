...
func TestParseIPFamilyFromAddressRejectsMalformedPorts(t *testing.T) {
	testCases := []string{
		"192.0.2.10:",
		"192.0.2.10:notaport",
		"192.0.2.10:65536",
	...
	for _, address := range testCases {
		t.Run(address, func(t *testing.T) {
			_, err := ParseIPFamilyFromAddress(address)
			assert.Error(t, err)
		})
	}
}
...
func TestUnifiedPodIPSelection(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:   "usable primary ignores unrelated interface lookup failure",
			family: IPFamilyUnspecified,
			podIP:  "192.0.2.32",
			hooks: &testIPForPodHooks{
				interfaces:       map[string]testInterfaceResult{StorageNetworkInterface: {}},
				interfaceNameErr: assert.AnError,
			},
			expected:        "192.0.2.32",
			expectedNameUse: 0,
			expectedCalls:   []string{StorageNetworkInterface},
		...
		{
			name:   "matching primary family ignores interface lookup failure",
			family: IPFamilyIPv4,
			podIP:  "192.0.2.33",
			hooks: &testIPForPodHooks{
				interfaces:       map[string]testInterfaceResult{StorageNetworkInterface: {}},
		...
		},
		{
			...
			expectedErrContains: []string{
				"no usable ipv6 address found on storage network interface",
				StorageNetworkInterface,
			},
			expectedNameUse: 0,
			expectedCalls:   []string{StorageNetworkInterface},
		...
		},
		{
			...
			expectedErrContains: []string{
				"no usable ipv6 address found on pod network for pod IP",
				"192.0.2.31",
			},
			expectedNameUse: 1,
			expectedCalls:   []string{StorageNetworkInterface, "eth0"},
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if len(testCase.expectedErrContains) > 0 {
				assert.Error(t, err)
				assert.Empty(t, ip)
				for _, expected := range testCase.expectedErrContains {
...
func TestGetIPForPodByNetworkAndFamilyFallbacks(t *testing.T) {
	testCases := []struct {
		...
		{
			name:   "unusable primary is not replaced by sibling",
			family: IPFamilyIPv6,
			podIP:  "fe80::1",
			hooks: &testIPForPodHooks{
				interfaces: map[string]testInterfaceResult{
					StorageNetworkInterface: {},
					"synthetic0": {
						addrs: []net.Addr{&net.IPNet{IP: net.ParseIP("2001:db8::44")}},
					},
				},
				interfaceName: "synthetic0",
			},
			expectedError:   `invalid POD_IP "fe80::1"`,
			expectedNameUse: 0,
			expectedCalls:   []string{StorageNetworkInterface},
		},
		{
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if len(testCase.expectedErrorContains) > 0 {
				assert.Error(t, err)
				assert.Empty(t, ip)
				for _, expected := range testCase.expectedErrorContains {
					assert.ErrorContains(t, err, expected)
				}
...
func TestSelectIPByNetworkPreference(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			podIPs:                []string{"192.0.2.10"},
			expectError:           true,
			expectedErrorContains: []string{"storage candidates", "127.0.0.1", "fe80::1"},
		},
		{
		...
		},
		{
			...
			podIPs:                []string{},
			expectError:           true,
			expectedErrorContains: []string{"pod candidates"},
		},
		{
...
func TestGetIPForPodByNetwork(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expectedErrorContains: []string{
				"no usable IP address found on storage network interface",
				StorageNetworkInterface,
			},
			expectedCalls: []string{StorageNetworkInterface},
		},
		...
		},
		{
			...
			expectedError: `invalid POD_IP "0.0.0.0"`,
			expectedCalls: []string{StorageNetworkInterface},
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			switch {
			case len(testCase.expectedErrorContains) > 0:
				assert.Error(t, err)
				assert.Empty(t, ip)
				for _, expected := range testCase.expectedErrorContains {
					assert.ErrorContains(t, err, expected)
...
func TestPublicPodIPSelectors(t *testing.T) {
	ip, err := GetIPForPodByNetworkAndFamily(IPFamily("invalid"))
	assert.EqualError(t, err, `invalid IP family "invalid"`)
	assert.Empty(t, ip)
}
